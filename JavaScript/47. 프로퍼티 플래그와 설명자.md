...
`writable`플래그를 사용해 `user.name`에 값을 쓰지 못하도록 해봅시다.
...
❕에러는 엄격 모드에서만 발생합니다.   
비 엄격 모드에선 읽기 전용 프로퍼티에 값을 쓰려고 해도 에러가 발생하지 않습니다. 다만 이때 값을 변경하는 것은 불가능합니다. 비 엄격모드에선 이와 같이 플래그에서 규칙을 위반하는 행위는 에러 없이 그냥 무시됩니다.   
...
```
그런데 특정 프로터티의 `enumerable`플래그 값을 `false`로 설정하면 `for..in`반복문에 나타나지 않게 할 수 있습니다. 커스텀 `toString`도 열거가 불가능하게 할 수 있습니다.
```
let user = {
  name: "John",
  toString() {
...
3. `writable: false`의 값을 `true`로 바꿀 수 없음(`true`를 `false`로 변경하는 것을 가능함).
...
위 샘플 코드처럼 `for..in`을 사용해 객체를 복사하면 심볼형 프로퍼티도 놓치게 됩니다. 하지만 `Object.getOwnPropertyDescriptors`는 심볼형 프로퍼티를 포함란 프로퍼티 설명자 전체를 반환합니다.
...
Object.seal(obj)   
새로운 프로퍼티 추가나 기존 프로퍼티를 삭제를 막아줍니다. 프로퍼티 전체에 `configurable: false`를 설정하는 것과 동일한 효과입니다.   
   
Object.freeze(obj)   
새로운 프로퍼티 추가나 기존 프로퍼티 삭제, 수정을 막아줍니다. 프로퍼티 전체에 `configurable: false, writable: false`를 설정하는 것과 동일한 효과입니다.   
   
아래 메서드은 위 세 가지 메서드를 사용해서 설정하 제약사항을 확인할 때 사용할 수 있습니다.   
...
Object.isFrozen(obj)   
프로퍼티 추가, 삭제, 변경이 불가능라고 모든 프로퍼티가 `configurable: false, writable: false`이면 `true`를 반환합니다.   
...
---
출처: 모던 JavaScript 튜토리얼 - 프로퍼티 플래그와 설명자
