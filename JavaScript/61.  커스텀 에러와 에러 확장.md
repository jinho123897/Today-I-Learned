...
이렇게 된면, `readUser`를 호출하는 바깥 코드에선 `instancof ReadError` 딱 하나만 확인하면 됩니다. 에러 처리 분기문을 여러 만들 필요가 없어집니다.   
   
'예외 감싸기'란 이름은 종류별 에러를 좀 더 추상적인 에러, `ReadError`에 하나로 모아 처리하기 때문에 붙여졌습니다. 이런 기법은 객체 지향 프로그래밍에서 널리 쓰이는 패턴입니다.   



---
출처: 모던 JavaScript 튜토리얼 - 커스텀 에러와 에러 확장
