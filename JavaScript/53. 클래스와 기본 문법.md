...
클래스 표현식에 이름을 붙이면, 이 이름은 오직 클래스 내부에선만 사용할 수 있습니다.
```
...
리터럴을 사용해 만든 객체처럼 클래스도 getter나 setter, 계산된 프로퍼티를 지원합니다.   
...
new User().sayHi();
```
계산된 메서드 이름은 리터럴 객체와 유사한 형태를 따기 때문에 사용법을 외우기 쉽다는 장점이 있습니다.



## 클래스 필드

> ❗구식 브라우저에선 풀리필이 필요할 수 있습니다.   
클래스 필드는 최근에 더해진 기능입니다.   
   
지금까지 살펴본 예시엔 메서드가 하나만 있었습니다.   
   
'클래스 필드'라는 문법을 사용하면 어떤 종류의 프로퍼티도 클래스에 추가할 수 있습니다.   
   
클래수 `User`에 `name`프로퍼티를 추가해봅시다.
```
Class User {
  name = "보라";
...
mew User().sayHi(); // 보라님 안녕하세요!
```
클래스를 정의할 때 '<프로포티 이름> = <값>'을 써주면 간단히 클래스 필드를 만들 수 있습니다.   
...
함수 바인딩 챕터에서 살펴본 것처럼 자바스크립트에서 `this`는 동적으로 결정됩니다.   
...
```
이렇게 `this`의 컨텍스트를 알 수 없데 되는 문제를 '잃어버린 `this`라고 합니다.   
...
```
클래스 필드 `click = () => {...}`는 각 `Button`객체마다 독립적인 함수를 만들어주고 이 함수의 `this`를 해당 객체에 바인딩 시켜줍니다. 따라서 개발자는 `button.click`을 아무 곳에나 전달할 수 있고, `this`엔 항상 의도한 값이 들어가게 됩니다.   
...
---
출처: 모던 JavaScript 튜토리얼 - 클래스와 기본 문법
